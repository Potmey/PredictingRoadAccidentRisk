...
async function onTrain() {
  try {
    ...
    const numLayers = Number($('numLayers').value);
    const neuronsPerLayer = Number($('neuronsPerLayer').value);
    const dropout = Number($('dropout').value);
    
    setStatus('building model…');
    log(`Building model with ${arch} | Layers: ${numLayers} | Neurons: ${neuronsPerLayer} | Dropout: ${dropout}`);
    MODEL?.dispose?.();
    MODEL = buildModel(arch, featNames.length, 0.001, numLayers, neuronsPerLayer, dropout);
    ...
    setStatus('testing…');
    log('Evaluate accuracy on full test (thr=0.5)');
    ...
    READY = true;
    setStatus('done');
    log('Training finished. Simulation enabled.');
  } catch (e) {
    console.error(e);
    log('Error: ' + e.message);
    setStatus('error');
